<template>
    ...
    <section class="lg:container mx-auto mt-7 p-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-x-4 gap-y-6">
            ...
            <div v-for="meal in meals" :key="meal.idMeal" class="max-w-sm mx-auto md:mx-0 bg-white border border-gray-200 rounded-lg shadow">
                <a href="#">
                    <img class="rounded-t-lg" :src="meal.strMealThumb" :alt="meal.strMeal" />
                </a>
                <div class="p-5">
                    <a href="#">
                        <h5 class="maxLine-2 mb-2 text-2xl font-bold tracking-tight text-gray-900">{{ meal.strMeal }}</h5>
                    </a>
                    <p class="maxLines-3 mb-3 font-normal text-gray-700">{{ meal.strInstructions }}</p>
                    <router-link to="" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300">
                        Read more
